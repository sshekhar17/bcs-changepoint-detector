...
def check_ARL(N, Alpha, num_trials, seed=None):
    if seed is None: 
        seed = int(time()%10000)
    experiment_name = 'GaussianMeanARL'
    #### Source 
    m0 = 0 
    m1=0.0
    Source = Univariate_Gaussian_Source
    Source_kwargs = {'mean0':m0, 'mean1':m1, 'nc':None}
    #### FWD CS 
...
def check_Delay(N, nc, Delta, num_trials, seed=None, alpha=0.01, 
                    save_data=False, save_fig=False, xmax=12, 
                    linear_fit=True):
    if seed is None: 
        seed = int(time()%10000)
    m0 = 0 
